...
class TestSmokeRun:
    ...
    def test_esd_columns_only_in_plant_and_utilities(
        self, layer_dfs: dict[str, pd.DataFrame]
    ) -> None:
    ...
    def test_parquet_files_written(
        self, layer_dfs: dict[str, pd.DataFrame], tiny_run_cfg: RunConfig
    ) -> None:
...
class TestEsdInjection:
    ...
    def test_phase_ordering(self, esd_run: dict[str, pd.DataFrame]) -> None:
        ...
        order = ["TRIP", "DEPRESSURE", "COMPRESSOR_TRIP", "UTILITIES_DOWN", "HOLD", "RECOVERY"]
        first_seen = []
        for ph in order:
    ...
    def test_flare_hp_spikes_during_depressure_window(
        self, esd_run: dict[str, pd.DataFrame]
    ) -> None:
...
class TestGasLockInjection:
    ...
    def test_gas_lock_zeros_oil(self, gaslock_run: dict[str, pd.DataFrame]) -> None:
        ...
        gl_rows = wells[(wells["well_id"] == "LLL-002") & (wells["well_state"] == "GAS_LOCK")]
    ...
    def test_gas_lock_recovers_after_window(self, gaslock_run: dict[str, pd.DataFrame]) -> None:
        ...
        post = wells[
            (wells["well_id"] == "LLL-002") & (wells["timestamp"] >= start + timedelta(hours=11))
        ]
        assert not post.empty
        assert (post["well_state"] != "GAS_LOCK").all()
        assert (post["well_state"].isin({"FLOWBACK", "PRODUCING"})).all()

    def test_other_layers_unaffected_by_well_event(
        self, gaslock_run: dict[str, pd.DataFrame]
    ) -> None:
